...
def pytest_sessionfinish(session, exitstatus):
    if importlib.util.find_spec("evalu8rpackage"):
        try:
            subprocess.check_call(
                [sys.executable, "-m", "pip", "uninstall", "-y", "evalu8rpackage"]
            )
            shutil.rmtree("tests/test_package/build")
            shutil.rmtree("tests/test_package/evalu8rpackage.egg-info")
        except subprocess.CalledProcessError as ex:
            warnings.warn(f"Error occurred uninstalling evalu8rpackage: {ex}")
