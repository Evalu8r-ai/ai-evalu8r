...
def test_web_browser_navigation():
    ...
    def is_evalu8rwebsite(page: str) -> bool:
        return (
            'RootWebArea "Evalu8r" '
            in page
        )

    def is_evalu8rrepo(page: str) -> bool:
        return (
            'RootWebArea "GitHub -: Evalu8r: A framework for large language model evaluations"'
            in page
        )
    ...
    def check_tool_call_page(tool: str, page: Literal["website", "repo"]) -> None:
        ...
        assert response
        if page == "website":
            assert is_evalu8rwebsite(response.text)
        elif page == "repo":
            assert is_evalu8rrepo(response.text)
