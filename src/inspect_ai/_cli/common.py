...
from evalu8r_ai._util.display import init_display_type
...
def log_level_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        ...
        envvar="EVALU8R_LOG_LEVEL_TRANSCRIPT",
...
def common_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        ...
        envvar="EVALU8R_NO_ANSI",
    )
    @click.option(
        "--debug", is_flag=True, envvar="EVALU8R_DEBUG", help="Wait to attach debugger"
    )
    @click.option(
        "--debug-port",
        default=5678,
        envvar="EVALU8R_DEBUG_PORT",
