...
from evalu8r_ai._util.logger import init_logger
from evalu8r_ai._view.server import view_server
...
def view_acquire_port(port: int) -> None:
    ...
    if pid_file.exists():
        ...
        try:
            ...
            p.terminate()
            display().print(
                f"Terminating existing evalu8r view command using port {port}"
            )
            p.wait(WAIT_SECONDS)
