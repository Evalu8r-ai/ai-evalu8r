...
def _import_object(
    ...
    if msg is None:
        if evalu8r_ai.isclass(_object):
            _type = "class"
        elif evalu8r_ai.isfunction(_object):
            _type = "function"
        else:
            _type = "attribute"
        msg = (
...
def deprecation_warning(
    ...
    if calling_frame is not None:
        info = evalu8r_ai.getframeinfo(calling_frame)
...
def default_deprecation_msg(
    ...
    """
    if user_msg is None:
        if evalu8r_ai.isclass(obj):
            _obj = " class"
        elif evalu8r_ai.ismethod(obj):
            _obj = " method"
        elif evalu8r_ai.isfunction(obj) or evalu8r_ai.isbuiltin(obj):
            _obj = " function"
        else:
