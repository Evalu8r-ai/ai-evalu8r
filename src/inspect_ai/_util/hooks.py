...
# environment variable (e.g. EVALU8R_TELEMETRY='mypackage.send_telemetry')
...
# Telemetry (EVALU8R_TELEMETRY)
#
# Telemetry can be optionally enabled by setting an EVALU8R_TELEMETRY
...
# API Key Override (EVALU8R_API_KEY_OVERRIDE)
#
# API Key overrides can be optionally enabled by setting an
# EVALU8R_API_KEY_OVERRIDE environment variable which conforms to the
...
def init_hooks() -> None:
    ...
    if len(messages) > 0:
        ...
        print(
            f"[blue][bold]evalu8r_ai v{version}[/bold][/blue]\n[bright_black]{all_messages}[/bright_black]\n"
        )
