...
class LogHandler(RichHandler):
    def __init__(self, levelno: int, transcript_levelno: int) -> None:
        ...
        file_logger = os.environ.get("EVALU8R_PY_LOGGER_FILE", None)
        ...
        file_logger_level = os.environ.get("EVALU8R_PY_LOGGER_LEVEL", "")
...
def init_logger(
    ...
    log_level = (
        log_level if log_level else os.getenv("EVALU8R_LOG_LEVEL", DEFAULT_LOG_LEVEL)
    ).upper()
    ...
    log_level_transcript = (
        ...
        else os.getenv("EVALU8R_LOG_LEVEL_TRANSCRIPT", DEFAULT_LOG_LEVEL_TRANSCRIPT)
    ).upper()
...
def notify_logger_record(record: LogRecord, write: bool) -> None:
    from evalu8r_ai.log._message import LoggingMessage
    from evalu8r_ai.log._transcript import LoggerEvent, transcript
