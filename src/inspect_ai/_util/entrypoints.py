...
def ensure_entry_points(package: str | None = None) -> None:
    ...
    global _evalu8rai_eps_loaded_all
    if _evalu8rai_eps_loaded_all:
        return None

    # have we already loaded entry points for this package?
    if package in _evalu8rai_eps_loaded:
        return None

    # enumerate entry points
    eps = entry_points(group="evalu8r_ai")
    for ep in eps:
        try:
...
# evalu8r extension entry points
_evalu8rai_eps_loaded: list[str] = []
_evalu8rai_eps_loaded_all: bool = False
