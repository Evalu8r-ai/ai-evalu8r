...
EVALU8R_LOG_DIR_VAR = "EVALU8R_LOG_DIR"
...
def init_dotenv() -> None:
    ...
    if dotenv_file:
        # is there an EVALU8R_LOG_DIR currently in the environment? (we will give it preference)
        environment_log_dir = os.environ.get(EVALU8R_LOG_DIR_VAR, None)
        dotenv_log_dir = dotenv_values(dotenv_file).get(EVALU8R_LOG_DIR_VAR, None)
        ...
        if environment_log_dir == dotenv_log_dir or not environment_log_dir:
            ...
            evalu8rlog_dir = absolute_file_path(environment_log_dir)
        ...
        if evalu8rlog_dir:
            os.environ[EVALU8R_LOG_DIR_VAR] = evalu8rlog_dir
