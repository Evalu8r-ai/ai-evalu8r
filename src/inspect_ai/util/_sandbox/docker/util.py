...
from evalu8r_ai._util.constants import SANDBOX
...
def task_project_name(task: str) -> str:
    ...
    return f"evalu8r-{task}-i{uuid().lower()}"


evalu8rproject_pattern = r"^evalu8r-[a-z\d\-_]*-i[a-z\d]{22}$"


def is_evalu8rproject(name: str) -> bool:
    return re.match(evalu8rproject_pattern, name) is not None
