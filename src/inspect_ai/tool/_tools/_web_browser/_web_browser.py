...
from evalu8r_ai.tool._tool import Tool, ToolError, tool
from evalu8r_ai.tool._tool_call import ToolCall, ToolCallContent, ToolCallView
from evalu8r_ai.tool._tool_info import parse_tool_info
from evalu8r_ai.tool._tool_with import tool_with
from evalu8r_ai.util._sandbox import SandboxEnvironment, sandbox_with
from evalu8r_ai.util._sandbox.docker.internal import EVALU8R_WEB_BROWSER_IMAGE_DOCKERHUB
from evalu8r_ai.util._store import store
...
async def web_browser_sandbox() -> SandboxEnvironment:
    ...
    if sb:
        ...
        msg = dedent(f"""
                The web browser service was not found in any of the sandboxes for this sample. Please add the web browser service to your configuration. For example, the following Docker compose file uses the {EVALU8R_WEB_BROWSER_IMAGE_DOCKERHUB} image as its default sandbox:

                services:
                  default:
                    image: "{EVALU8R_WEB_BROWSER_IMAGE_DOCKERHUB}"
                    init: true
