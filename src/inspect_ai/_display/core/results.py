...
from evalu8r_ai.log import EvalStats
from evalu8r_ai.log._log import EvalScore, rich_traceback
    ...
    if samples_completed > 0:
        ...
        if task_can_retry(profile):
            message = (
                ...
                + f"[bold][{theme.light}]evalu8r eval-retry {log_location}[/{theme.light}][/bold]"
            )
        else:
