...
from evalu8r_ai.solver import Generate, Solver, TaskState
from evalu8r_ai.tool import ToolCall, ToolChoice, ToolInfo
...
def langchain_solver(agent: LangChainAgent) -> Solver:
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        # create the evalu8r model api bridge
        llm = Evalu8rChatModel()
...
class Evalu8rChatModel(BaseChatModel):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        # evalu8r uses async exclusively
    ...
    async def _agenerate(
        ...
        **kwargs: dict[str, Any],
        ...
        # generate
        input = [as_evalu8rmessage(message) for message in messages]
...
def as_evalu8rmessage(message: BaseMessage) -> ChatMessage:
    if isinstance(message, SystemMessage):
        return ChatMessageSystem(content=as_evalu8rcontent(message.content))
    elif isinstance(message, HumanMessage):
        return ChatMessageUser(content=as_evalu8rcontent(message.content))
    elif isinstance(message, AIMessage):
        return ChatMessageAssistant(
            content=as_evalu8rcontent(message.content),
            tool_calls=(
                [
                    ToolCall(
                        type="function",
