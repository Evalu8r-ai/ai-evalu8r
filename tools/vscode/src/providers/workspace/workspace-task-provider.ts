...
export function activateWorkspaceTaskProvider(evalu8rManager: Evalu8rManager, context: ExtensionContext) {

  // The task manager
  const taskManager = new WorkspaceTaskManager();

  // If the interpreter changes, refresh the tasks
  context.subscriptions.push(evalu8rManager.onEvalu8rChanged(async (e: Evalu8rChangedEvent) => {
    if (e.available) {
...
export class WorkspaceTaskManager {
  ...
  public async refresh() {
    ...
    try {
      const tasks = await evalu8rTaskData(workspace);
      this.setTasks(tasks);
    } catch (err: unknown) {
      console.log("Unable to read evalu8r task data.");
      console.error(err);
    }
...
async function evalu8rTaskData(folder: AbsolutePath) {
