...
export async function activateTaskOutline(
  ...
  const checkEvalu8r = async () => {
    const evalu8rAvailable = evalu8rVersion() !== null;
    await commands.executeCommand(
      "setContext",
      "evalu8r_ai.task-outline-view.noEvalu8r",
      !evalu8rAvailable
    );
    if (evalu8rAvailable) {
  ...
  context.subscriptions.push(
    evalu8rManager.onEvalu8rChanged(async () => {
      await checkEvalu8r();
    })
  );
  await checkEvalu8r();
  ...
  return [
    [
      ...
      new RunSelectedEvalCommand(evalu8rEvalMgr),
      new DebugSelectedEvalCommand(evalu8rEvalMgr),
      new EditSelectedTaskCommand(tree, evalu8rLogviewManager, activeTaskManager),
...
export class TaskOutLineTreeDataProvider
  ...
  public static readonly viewType = "evalu8r_ai.task-outline-view";
  constructor(
      ...
      this.workspaceMgr.onTasksChanged(
        throttle(
          async (e: TasksChangedEvent) => {
            ...
            await commands.executeCommand(
              "setContext",
              "evalu8r_ai.task-outline-view.tasksLoaded",
              true
            );
            await commands.executeCommand(
              "setContext",
              "evalu8r_ai.task-outline-view.noTasks",
              e.tasks?.length === 0
            );
          },
          500,
  ...
  private getNodes(tree: TaskPath[], parent?: TaskTreeItem): TaskTreeItem[] {
    const mode =
      workspace.getConfiguration("evalu8r_ai").get("taskListView") || "tree";
