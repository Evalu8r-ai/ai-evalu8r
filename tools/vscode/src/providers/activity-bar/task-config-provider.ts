...
export type NoEvalu8rCmd = {
  command: "noEvalu8r";
};

export class TaskConfigurationProvider implements WebviewViewProvider {
  public static readonly viewType = "evalu8r_ai.task-configuration";
  ...
  public async resolveWebviewView(webviewView: WebviewView) {
    ...
    this.disposables_.push(this.evalu8rManager_.onEvalu8rChanged(async () => {
...
const updateSidebarState = async (taskInfo?: DocumentTaskInfo) => {
  await commands.executeCommand(
    "setContext",
    "evalu8r_ai.task-configuration.task-active",
    taskInfo !== undefined
  );
};
