...
import { kEvalu8rEnvValues } from "../evalu8r/evalu8r-constants";
import { evalu8rVersion } from "../../evalu8r";
import { Evalu8rChangedEvent, Evalu8rManager } from "../evalu8r/evalu8r-manager";
...
const kEvalu8rModels: Record<string, string> = {
  "openai": "0.3.8",
  "anthropic": "0.3.8",
  "google": "0.3.8",
  "mistral": "0.3.8",
...
const evalu8rModels = () => {
  ...
  return Object.keys(kEvalu8rModels).filter((key) => {
    const ver = kEvalu8rModels[key];
    if (descriptor !== null) {
...
export class EnvConfigurationProvider implements WebviewViewProvider {
  public static readonly viewType = "evalu8r_ai.env-configuration-view";
  ...
  constructor(
    ...
    private readonly evalu8rManager_: Evalu8rManager
  ) { }
  ...
  public resolveWebviewView(webviewView: WebviewView) {
    ...
    this.disposables_.push(this.evalu8rManager_.onEvalu8rChanged(async (e: Evalu8rChangedEvent) => {
      if (e.available) {
        await initMsg();
      } else {
        await noEvalu8rMsg();
  ...
  private htmlForWebview(webview: Webview) {
    ...
    const modelOptions = evalu8rModels().map((model) => {
...
const envToConfig = (envManager: WorkspaceEnvManager) => {
  ...
  const providerModelStr = env[kEvalu8rEnvValues.providerModel];
  if (providerModelStr) {
  ...
  const logLevel = env[kEvalu8rEnvValues.logLevel];
  if (logLevel) {
    config.logLevel = logLevel;
  }

  const logDir = env[kEvalu8rEnvValues.logDir];
  if (logDir) {
    config.logDir = logDir;
  }

  const maxConnections = env[kEvalu8rEnvValues.connections];
  if (maxConnections) {
    config.maxConnections = maxConnections;
  }

  const maxRetries = env[kEvalu8rEnvValues.retries];
  if (maxRetries) {
    config.maxRetries = maxRetries;
  }

  const timeout = env[kEvalu8rEnvValues.timeout];
  if (timeout) {
    config.timeout = timeout;
  }

  const modelBaseUrl = env[kEvalu8rEnvValues.modelBaseUrl];
  if (modelBaseUrl) {
...
const configToEnv = (config: EnvConfiguration): Record<string, string> => {
  ...
  if (config.provider && config.model) {
    env[kEvalu8rEnvValues.providerModel] = `${config.provider}/${config.model}`;
  } else {
    env[kEvalu8rEnvValues.providerModel] = "";
  }

  env[kEvalu8rEnvValues.logLevel] = config.logLevel || "";
  env[kEvalu8rEnvValues.logDir] = config.logDir || "";
  env[kEvalu8rEnvValues.connections] = config.maxConnections || "";
  env[kEvalu8rEnvValues.retries] = config.maxRetries || "";
  env[kEvalu8rEnvValues.timeout] = config.timeout || "";
  env[kEvalu8rEnvValues.modelBaseUrl] = config.modelBaseUrl || "";
