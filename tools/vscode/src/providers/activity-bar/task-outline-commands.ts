...
export class ShowTaskTree implements Command {
  ...
  async execute(): Promise<void> {
    await workspace
      .getConfiguration("evalu8r_ai")
      .update("taskListView", "tree", ConfigurationTarget.Global);
    return this.provider_.refresh();
  }
  private static readonly id = "evalu8r_ai.taskOutlineTree";
...
export class ShowTaskList implements Command {
  ...
  private static readonly id = "evalu8r_ai.taskOutlineList";
  public readonly id = ShowTaskList.id;
}

export class RunSelectedEvalCommand implements Command {
  constructor(private readonly evalu8rEvalMgr_: Evalu8rEvalManager) { }
  async execute(treeItem: TaskTreeItem): Promise<void> {
    ...
    const evalPromise = this.evalu8rEvalMgr_.startEval(
      toAbsolutePath(path),
      task,
      false
    );
    const resumeFocusCommand = "evalu8r_ai.task-outline-view.focus";
    scheduleReturnFocus(resumeFocusCommand);
    await evalPromise;
  }
  private static readonly id = "evalu8r_ai.runSelectedTask";
...
export class DebugSelectedEvalCommand implements Command {
  ...
  async execute(treeItem: TaskTreeItem): Promise<void> {
    ...
    const task =
      treeItem.taskPath.type === "task" ? treeItem.taskPath.name : undefined;
    await this.evalu8rEvalMgr_.startEval(toAbsolutePath(path), task, true);
  }
  private static readonly id = "evalu8r_ai.debugSelectedTask";
...
export class EditSelectedTaskCommand implements Command {
  constructor(
    ...
    private evalu8rLogviewManager_: Evalu8rViewManager,
  ...
  async execute() {
    ...
    if (this.tree_.selection.length === 1) {
      ...
      const logViewColumn = this.evalu8rLogviewManager_.viewColumn();
  ...
  public static readonly id = "evalu8r_ai.editSelectedTask";
...
export class CreateTaskCommand implements Command {
  ...
  private static readonly id = "evalu8r_ai.createTask";
