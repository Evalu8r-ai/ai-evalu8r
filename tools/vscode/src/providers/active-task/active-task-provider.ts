...
export function activateActiveTaskProvider(
  evalu8rEvalManager: Evalu8rEvalManager,
  context: ExtensionContext
): [Command[], ActiveTaskManager] {
  const activeTaskManager = new ActiveTaskManager(context);

  const commands = [
    new RunActiveTaskCommand(activeTaskManager, evalu8rEvalManager),
    new DebugActiveTaskCommand(activeTaskManager, evalu8rEvalManager),
  ];
