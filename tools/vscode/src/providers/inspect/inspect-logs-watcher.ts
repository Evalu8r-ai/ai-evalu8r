...
import { evalu8rLastEvalPaths } from "../../evalu8r/props";
import { existsSync, readFileSync, statSync } from "fs";
import { log } from "../../core/log";
import { WorkspaceStateManager } from "../workspace/workspace-state-provider";
import { withMinimumEvalu8rVersion } from "../../evalu8r/version";
import { kEvalu8rChangeEvalSignalVersion } from "./evalu8r-constants";
import { resolveToUri } from "../../core/uri";

export interface Evalu8rLogCreatedEvent {
  log: Uri
  externalWorkspace: boolean;
}

export class Evalu8rLogsWatcher implements Disposable {
  constructor(
    ...
    const evalSignalFiles = evalu8rLastEvalPaths().map(path => path.path);
      ...
      for (const evalSignalFile of evalSignalFiles) {
        if (existsSync(evalSignalFile)) {
          ...
          if (updated > this.lastEval_) {
            ...
            withMinimumEvalu8rVersion(
              kEvalu8rChangeEvalSignalVersion,
              () => {
                // 0.3.10- or later
            ...
            if (evalLogPath !== undefined) {
              ...
              try {
                ...
                this.onEvalu8rLogCreated_.fire({ log: logUri, externalWorkspace });
              } catch (error) {
  ...
  private readonly onEvalu8rLogCreated_ =
    new EventEmitter<Evalu8rLogCreatedEvent>();
  public readonly onEvalu8rLogCreated: Event<Evalu8rLogCreatedEvent> =
    this.onEvalu8rLogCreated_.event;
