...
import { kPythonPackageName } from "../../evalu8r/props";
import { removeFilesSync } from "../../core/file";


export function activateEvalu8rCommands(
  stateManager: WorkspaceStateManager,
  context: ExtensionContext) {
  const evalu8rCommands = new Evalu8rCommandDispatcher(stateManager);
  context.subscriptions.push(evalu8rCommands);
}


export class Evalu8rCommandDispatcher implements Disposable {

  constructor(stateManager: WorkspaceStateManager) {
    // init commands dir and remove any existing commands
    this.commandsDir_ = evalu8rCommandsDir(stateManager);
...
function evalu8rCommandsDir(stateManager: WorkspaceStateManager): string {
